"""4元 AEP + 流形空间映射 (MSM) 测试

配置: 4元, d=0.5λ, fc=2.45GHz, 位置固定, 仅优化相位
流程:
  1. CMA-ES 优化 AEP 粗模型 (4维相位)
  2. HFSS 细模型仿真
  3. MSM 迭代 (替代模型 + CMA-ES + Broyden S 更新)

用法:
  1. 先生成 AEP: python test_aep_4elem.py
  2. 再跑 MSM: python test_msm_4elem.py
"""
